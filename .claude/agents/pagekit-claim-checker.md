...
3. For each flagged line, propose a sharper, more accurate rewrite (or a cut). Precision over intensity. If a rewrite feels smaller, that is usually correct.
...
7. Save the corrected draft to `runs/<run-name>/first-page-draft-corrected.md` (only if any line was rewritten).
