...
If a pattern shows up in claim checking on every run (for example, editorial voice or rule-of-three cadence on every page), the fix belongs in the page-argument-shape drafting constraints and in `../../pagekit/references/anti-slop.md`, not in the claim-check step. Claim checking is a safety net. It should not be the primary place a category of problems gets caught.
